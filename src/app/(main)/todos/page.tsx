  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        ...
        {/*
          이 화면의 유일한 "할 일 추가" 버튼이며 어떤 상태에서도 자리를 비우지 않는다.
          빈 상태일 때만 EmptyState 안으로 옮겨봤더니, 검색어를 입력해 결과가 없어지는
          순간 버튼이 본문 중앙에서 여기로 튀었다(디바운스 때문에 타이핑을 멈춘 뒤에야
          움직여서 원인도 짐작하기 어렵다). 버튼 위치는 예측 가능한 편이 낫다.
        */}
      ...
      {query.isError ? (
        ...
        keyword ? (
          ...
          // 여기에 버튼을 두지 않는다(위 헤더 주석 참고). 대신 문구가 버튼을 가리킨다.
          <EmptyState
            icon={<Inbox />}
            title="아직 할 일이 없어요"
            description="위쪽 '할 일 추가' 버튼으로 첫 번째 할 일을 만들어 보세요."
          />
        )
      ) : data ? (
        <>
